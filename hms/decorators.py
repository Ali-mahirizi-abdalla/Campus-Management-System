...
def super_admin_required(view_func):
    return role_required(allowed_roles=['Super Admin', 'SUPER_ADMIN'])(view_func)
...
def welfare_officer_required(view_func):
    return role_required(allowed_roles=['Welfare Officer', 'WELFARE_OFFICER', 'SUPER_ADMIN'])(view_func)

def hostel_manager_required(view_func):
    return role_required(allowed_roles=['Hostel Manager', 'HOSTEL_MANAGER', 'SUPER_ADMIN'])(view_func)

def kitchen_manager_required(view_func):
    return role_required(allowed_roles=['Kitchen Manager', 'KITCHEN_MANAGER', 'SUPER_ADMIN'])(view_func)

def security_required(view_func):
    return role_required(allowed_roles=['Security', 'SECURITY_OFFICER', 'EMERGENCY_COORDINATOR', 'SUPER_ADMIN'])(view_func)
